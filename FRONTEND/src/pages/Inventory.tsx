...
import { AlertTriangle, Package, TrendingDown, Plus, Search } from 'lucide-react';
...
const Inventory: React.FC = () => {
  ...
  const { stockAlerts, fetchStockAlerts } = usePurchaseOrderStore();
  ...
  const filtered = products.filter(p => {
    ...
    if (filter === 'alerts') return p.stock <= p.minStock;
    return true;
  });
          ...
          <table className="min-w-full bg-white text-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.length === 0 ? (
                ...
                return (
                  <tr key={product.id} className={`hover:bg-gray-50 ${product.stock <= 0 ? 'bg-red-50/40' : product.stock <= product.minStock ? 'bg-yellow-50/40' : ''}`}>
