...
| `/quotes` | Quotes | seller, admin (feature `quotes`) |
...
- **Ciclo de suscripción (v1.11.0)**: `tenants.paidUntil` (DATEONLY, null = cortesía/legado exento del ciclo) + `suspendedReason` (trial_expired|nonpayment|manual — un pago reactiva las dos primeras, NUNCA la manual). Registro manual de pagos por superadmin: `POST /tenants/:id/payments` (crea PlanPayment approved source=superadmin con REC-#### y extiende paidUntil vía `applyPaymentToTenant`, misma ruta de código que aprobar un pago reportado). `plan_payments` ganó source/method/months/paidAt/periodStart/periodEnd/notes. Job `subscriptionRenewal` (dailyScheduler): avisa al tenant a warnDays y 1 día, gracia configurable, suspende por no pago con push a ambos lados; `renewal_warn_days`/`grace_days` en platform_settings (default 5/5). Sección **Finanzas** en superadmin: MRR, cobrado/mes, morosos, próximos vencimientos, sin-fecha (lista de trabajo post-deploy), histórico por tenant; modal "Registrar pago" (botón $ en la fila y en cada tabla). Lógica de fechas pura en `modules/tenant/subscription.ts` (testeada: clamp fin de mes, ancla vigente/vencido). **Banner en el Sidebar (sep 2026)**: antes el aviso de renovación solo llegaba por push/email — nada lo mostraba dentro de la app. `Sidebar.tsx` (solo admin, `paidUntil` de `GET /tenants/me`, mismo cálculo que `subscriptionRenewal.ts` en el backend) agrega un banner igual al de trial (mismo componente/posición, mutuamente excluyentes — un tenant no está en ambos planes a la vez): ámbar si faltan ≤7 días, rojo si ya venció. Un tenant realmente suspendido por no pago no lo ve — login y `tenantScope` ya lo bloquean antes de llegar al Sidebar; esto cubre la ventana en que todavía puede entrar (por vencer, o vencido y en gracia). `paidUntil` agregado al tipo `Tenant` del frontend (antes solo `plan`/`trialEndsAt`, faltaba este campo aunque el backend ya lo devolvía). El cálculo (antes duplicado en el propio `Sidebar.tsx`) se extrajo a `hooks/useTenantRenewalStatus.ts` para compartirlo con `Header.tsx`: en mobile no hay espacio para el bloque lateral, así que ahí es una franja angosta siempre visible debajo de la barra superior (no escondida en el menú de usuario, que solo se ve si el admin lo abre) — mismos mensajes/colores, mismo destino `/billing`. **Página `/billing` separada de Configuración (sep 2026)**: "Plan y límites" + "Pagar plan (Bre-B)" + historial de pagos vivían dentro de `TenantSettings.tsx` (`/settings`), mezclados con identidad/branding — el usuario lo reportó como que "se veía mal" ahí. Se extrajeron a `pages/Billing.tsx` (ruta `/billing`, mismo `AdminRoute`), mismo criterio que ya se aplicó en el panel de superadmin al separar Finanzas de Configuración. `TenantSettings.tsx` queda solo con Identidad (nombre/logo/NIT/dirección/ciudad) y Branding (color). Todos los links que apuntaban a `/settings` para "ver mi plan" (banners de trial/renovación en Sidebar/Header, `FeatureGate.tsx`) se actualizaron a `/billing`. Ítem "Facturación" nuevo en `Sidebar.tsx`/`BottomNav.tsx`, junto a "Configuración". **Bug real corregido de paso** (reportado con captura apenas se separó la página): el selector "Plan a pagar" arrancaba siempre en `'basic'` hardcodeado, sin importar el plan real del tenant — un tenant Pro veía "Básico" preseleccionado. Ahora `payForm.plan` se inicializa con `tenantInfo.plan` (si no es `trial`) al cargar. **Correo de facturación + banner de datos incompletos (sep 2026)**: reportado con captura de un tenant real en producción cuyo recibo salía con NIT/dirección/ciudad en blanco (`—`, el fallback del recibo funcionaba bien, el dato simplemente nunca se llenó). `tenants.contactEmail` ya existía (v1.12.0, destino de los avisos de renovación por email) pero **solo el superadmin podía fijarlo** — `updateThemeSchema` (self-service, `PUT /tenants/me/theme`) no lo incluía. Se agregó al allowlist (`z.string().email()`, mismo patrón `'' → null` que ya usa `updateTenantSchema.contactEmail`) y a `getInfo()`/`updateTheme()` en `tenant.service.ts`. `Billing.tsx` gana: (1) banner ámbar arriba de todo si falta NIT/dirección/ciudad/correo, listando qué falta, con link a `/settings` para NIT/dirección/ciudad (siguen viviendo ahí, son también fiscales-del-tenant-para-sus-propios-PDF, no solo para Merco) y (2) card propia "Correo de facturación" con el campo editable inline (`updateMyTheme({contactEmail})`), ya que no encaja en Identidad — es específicamente el canal de avisos de vencimiento de Merco, no la identidad del negocio del tenant. **Bug real cerrado de paso**: el handler de `updateTheme` llamaba `updateThemeSchema.parse(req.body)` directo (no `validateSchema()`) — un `ZodError` no cae en ningún catch que lo traduzca a 400/422, así que cualquier input inválido (nombre vacío, color mal formado, y ahora un correo mal escrito) le devolvía al usuario un "Internal Server Error" (500) genérico en vez de un mensaje claro. Preexistía desde que se creó el endpoint; se hizo visible ahora porque un correo con typo es un error de usuario mucho más probable que los campos anteriores. Corregido a `validateSchema()` (helper que ya existía y usan otros controllers) → 422 con el mensaje real de Zod. Verificado con una prueba de integración real (Docker local): correo inválido → 422 limpio (antes 500); correo válido + nit/dirección/ciudad → se guardan y `GET /tenants/me` los refleja; intento de escalar `{plan:'enterprise', maxUsers:9999}` junto con el correo → el correo se guarda, plan/maxUsers se ignoran (el allowlist de la Fase A sigue intacto); `contactEmail: ""` → limpia el campo a `null` sin error
