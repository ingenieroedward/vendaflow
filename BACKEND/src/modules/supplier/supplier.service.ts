...
export class SupplierService {
  async createSupplier(supplierData: CreateSupplierDto): Promise<SupplierResponseDto> {
    const validatedData = validateSchema(createSupplierSchema, supplierData);

    // Check if supplier already exists
    const existingSupplier = await Supplier.findOne({ where: { name: validatedData.name } });
    if (existingSupplier) {
      throw new ConflictError('Supplier with this name already exists');
    }

    const supplier = await Supplier.create(validatedData as SupplierAttributes);
    return this.mapToResponseDto(supplier);
  }

  async getAllSuppliers(query: PaginationQuery): Promise<SuppliersListResponseDto> {
  ...
  async updateSupplier(id: number, updateData: UpdateSupplierDto): Promise<SupplierResponseDto> {
